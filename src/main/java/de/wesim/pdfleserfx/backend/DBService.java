...
public class DBService {
    ...
    public BookSettings findDbEntryForFile(Path file) {
        ...
        try {
        var entry
                = this.jdbc_template.queryForObject("select * from book_settings where filename=?; ",
                        (result_set, row_num) -> {
    ...
    public int update(BookSettings current_settings) {
        var rows_updated = this.jdbc_template.update("""
                                  ...
                                  current_settings.last_read,
                                  current_settings.background_color,
                                  current_settings.current_page, 
                                  current_settings.crop_top,
                                  current_settings.crop_bottom,
                                  current_settings.crop_left,
                                  current_settings.crop_right, current_settings.filename);
        return rows_updated;
    }
}
